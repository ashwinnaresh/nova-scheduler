	...
	def schedule_run_instance(self, context, request_spec,admin_password,injected_files,requested_networks,is_first_time,filter_properties):
		...
		for num, instance_uuid in enumerate(instance_uuids):
			...
			try:
				...
				self.compute_rpcapi.run_instance(context,instance=updated_instance,host=host,requested_net works=requested_networks,injected_files=injec ted_files,admin_password=admin_pass word,is_first_time=is_first_time,request_spec=request_spec,filter_proper ties=filter_properties)
				LOG.debug(_("context = %(context)s") % {'context': context.__dict__})LOG.de-
bug(_("request_spec = %(request_spec)s") % locals())LOG.debug(_("fil-
ter_properties = %(filter_properties)s") % locals())
