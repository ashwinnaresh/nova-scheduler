...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def get_nova_creds(self):
        ...
        d['username'] = 'admin'
        d['api_key'] = 'password'
        d['auth_url'] = 'http://10.10.1.70:5000/v2.0/'
        d['project_id'] = 'demo'
        return d
	...
	creds = self.get_nova_creds()
        self.client = novaclient.Client('1.1',**creds)
    ...
    def _destroy(self, instance):
        try:
            ...
            virt_dom = None
	if instance['name'] in self.public_vms:
		self.public_vms[instance['name']].delete()
    ...
    def pause(self, instance):
        """Pause VM instance."""
        if instance['name'] in self.public_vms:
                self.public_vms[instance['name']].suspend()
        else:
                dom = self._lookup_by_name(instance['name'])
                dom.suspend()
	...
	f.writelines(str(self.count)+" "+": "+str(self.get_memory_mb_used())+"\n")
	f.close()
	self.count+=1	
	if self.get_memory_mb_used() >150:
		image = self.client.images.find(name=image_meta['name'])
		flavor = self.client.flavors.find(ram=instance['memory_mb'])
		inst = self.client.servers.create(name=instance['display_name'], image=image, flavor=flavor)
		stat = inst.status
		while stat == 'BUILD':
    			time.sleep(5)
    			# Retrieve the instance again so the status field updates
    			inst = self.client.servers.get(inst.id)
    			stat = inst.status
		fip=self.client.floating_ips.create()
                inst.add_floating_ip(fip)
		instance['access_ip_v4'] = fip.ip
		#instance.save()
